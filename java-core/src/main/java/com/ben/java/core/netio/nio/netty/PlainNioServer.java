			...
			while (iterator.hasNext()) {
				...
				try {
					if (key.isAcceptable()) { // 6.检查该事件是一个新的连接准备好接受。
						...
						client.register(selector, SelectionKey.OP_WRITE | SelectionKey.OP_READ, msg.duplicate()); // 7.接受客户端，并用
																													// selector
																													// 进行注册。
					...
					if (key.isWritable()) { // 8.检查 socket 是否准备好写数据。
						...
						while (buffer.hasRemaining()) {
							if (client.write(buffer) == 0) { // 9.将数据写入到所连接的客户端。如果网络饱和，连接是可写的，
